...
<-
Condition=condition(before(done(delimit_area)));
World=world([done(delimit_area)]);
...
+!convert_parametric_to_simple_formula(ParamLogicFormula,Variables,AssignmentSet,LogicFormula)
	...
	<-
		...
		if (earlier(SDate1,SDate2)) {LogicFormula=true;} 
		else 						{LogicFormula=false;}
	.
+!convert_parametric_to_simple_formula(ParamLogicFormula,Variables,AssignmentSet,LogicFormula)
	:
...
/**
 * Check if exists at least one assignment for each term in the variable list.
 */
...
+!unroll_assignment_to_check_variable( AssignmentSet, Variable, Bool )
	...
	<-
		...
		if (FreeVariable = Variable) 	{Bool = true} 
		else 							{!unroll_assignment_to_check_variable( Tail, Variable, Bool );}
	.
...
/*****PIANI AGGIUNTI DA PATRIZIA */


// after � vera se lo statement � presente nello stato del mondo
+!test_logic_formula(LogicFormula,World,Bool)
	:
	...
	.
// before � vera se lo statement non � presente nello stato del mondo	
+!test_logic_formula(LogicFormula,World,Bool)
	:
...
+!test_logic_formula(LogicFormula,World,Bool)
	...
	<-.println("33333");
		...
		.println(LogicFormula," : ",Bool,"in StateWorld",StateOfWorld);
	.
