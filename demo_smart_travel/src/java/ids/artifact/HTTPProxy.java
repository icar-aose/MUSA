...
public class HTTPProxy extends Artifact {
	...
	private class Server {
		...
		public Server() throws IOException {
			...
			if (debug) System.out.println("Opened the connection");
		}
	...
	private class Connection {
		...
		public Connection(Socket connection) {
			...
			try {
				...
				if (json_message.has("agent") && json_message.has("service")) {
					if (debug) System.out.println("well-formed");
					agent = json_message.getString("agent");
					service = json_message.getString("service");

					if (json_message.has("session")) {
						session = json_message.getString("session");
					}
					if (json_message.has("username")) {
						user = json_message.getString("username");
					}
					if (json_message.has("userrole")) {
				...
				if (debug) System.out.println("Read failed");
